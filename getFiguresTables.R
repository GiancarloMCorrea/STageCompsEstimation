...
allMethodsPropYear4 = aggregate(allMethodsPropYear3$MRE, list(YEAR = allMethodsPropYear3$YEAR, 
...
allMethodsPropYear5 = aggregate(allMethodsPropYear3$MRE, list(AGE = allMethodsPropYear3$AGE, 
								METHOD = allMethodsPropYear3$METHOD), mean)
allMethodsPropYear55 = aggregate(allMethodsPropYear3$MRE, list(AGE = allMethodsPropYear3$AGE, 
...
# allMethodsPropYear5$menleub = allMethodsPropYear5$x + allMethodsPropYear5$sdx
	...
	bitmap(paste0('FinalFigures/compareProportions4_', scenarioName, '.tiff'), height = 90, width = 190, units = 'mm', res = 900)
	print(ggplot(allMethodsPropYear5, aes(AGE, x, color = factor(METHOD))) +
			geom_errorbar(aes(ymin = x - sdx, ymax = x + sdx), width=.35) +
			geom_line(aes(color = factor(METHOD))) +
			geom_point(aes(color = factor(METHOD)), size = 1) +
			xlab('Age') +
			ylab('MRE') +
			facet_wrap( ~ factor(METHOD), ncol = 5) +
			theme_bw() +
...
allMethodsPropYear6 = aggregate(allMethodsPropYear3$MRE, list(YEAR = allMethodsPropYear3$YEAR, 
								METHOD = allMethodsPropYear3$METHOD), mean)
allMethodsPropYear66 = aggregate(allMethodsPropYear3$MRE, list(YEAR = allMethodsPropYear3$YEAR, 
								METHOD = allMethodsPropYear3$METHOD), sd)
allMethodsPropYear6$sdx = allMethodsPropYear66$x

	bitmap(paste0('FinalFigures/compareProportions5_', scenarioName, '.tiff'), height = 90, width = 190, units = 'mm', res = 900)
	print(ggplot(allMethodsPropYear6, aes(YEAR, x, color = factor(METHOD))) +
			geom_errorbar(aes(ymin = x - sdx, ymax = x + sdx), width=.35) +
			geom_line(aes(color = factor(METHOD))) +
			geom_point(aes(color = factor(METHOD)), size = 1) +
			xlab('Year') +
...
allMethodsPropYear44 = aggregate(allMethodsPropYear3$MRE, list(YEAR = allMethodsPropYear3$YEAR, 
...
#  ------------------------------------------------------------------------
# Get table 1: MRE per year per age
allMethodsPropYear3 = NULL
for(k in seq_along(allFiles22)){
	tmp = read.csv(file.path(selFolder2, allFiles22[k]))
	allMethodsPropYear3 = rbind(allMethodsPropYear3, tmp)
...
# Get table 2: MRE per age
allMethodsPropYear41 = aggregate(allMethodsPropYear4$x, list(AGE = allMethodsPropYear4$AGE, 
								METHOD = allMethodsPropYear4$METHOD), mean)
tab1 = cast(allMethodsPropYear41, METHOD ~ AGE)
write.csv(tab1, paste0('FinalTables/MRE_age_', scenarioName, '.csv'), row.names = FALSE)

# Get table 3: MRE per year per age
allMethodsPropYear42 = aggregate(allMethodsPropYear4$x, list(YEAR = allMethodsPropYear4$YEAR, 
								METHOD = allMethodsPropYear4$METHOD), mean)
tab2 = cast(allMethodsPropYear42, METHOD ~ YEAR)
write.csv(tab2, paste0('FinalTables/MRE_year_', scenarioName, '.csv'), row.names = FALSE)


# Get table 4: total average:
allMethodsPropYear43 = aggregate(allMethodsPropYear3$MRE, list(METHOD = allMethodsPropYear3$METHOD), mean)
write.csv(allMethodsPropYear43, paste0('FinalTables/MRE_all_', scenarioName, '.csv'), row.names = FALSE)


#  ------------------------------------------------------------------------
# Get table 1: MSE per year per age

allMethodsPropYear4 = aggregate(allMethodsPropYear3$MSE, list(YEAR = allMethodsPropYear3$YEAR, 
								AGE = allMethodsPropYear3$AGE, METHOD = allMethodsPropYear3$METHOD), mean)

# Get table 2: MSE per age
allMethodsPropYear41 = aggregate(allMethodsPropYear4$x, list(AGE = allMethodsPropYear4$AGE, 
								METHOD = allMethodsPropYear4$METHOD), mean)
tab3 = cast(allMethodsPropYear41, METHOD ~ AGE)
write.csv(tab3, paste0('FinalTables/MSE_age_', scenarioName, '.csv'), row.names = FALSE)


# Get table 3: MSE per year per age
allMethodsPropYear42 = aggregate(allMethodsPropYear4$x, list(YEAR = allMethodsPropYear4$YEAR, 
								METHOD = allMethodsPropYear4$METHOD), mean)
tab4 = cast(allMethodsPropYear42, METHOD ~ YEAR)
write.csv(tab4, paste0('FinalTables/MSE_year_', scenarioName, '.csv'), row.names = FALSE)

# Get table 4: total average:
allMethodsPropYear44 = aggregate(allMethodsPropYear3$MSE, list(METHOD = allMethodsPropYear3$METHOD), mean)
write.csv(allMethodsPropYear44, paste0('FinalTables/MSE_all_', scenarioName, '.csv'), row.names = FALSE)
