...
# chunk to estimate c_a_i ------------------------------------------------------------
# tempo22 = tempo2
# tempo22 = as.data.frame(tempo22)
# tempo22$ID_HAUL = rownames(tempo22)
# met2df = melt(tempo22, id = c('ID_HAUL'))
# names(met2df) = c('ID_HAUL', 'AGE', 'C_A_I')
# lenstring = nchar(met2df$ID_HAUL)
# met2df$YEAR = substr(met2df$ID_HAUL, 1, 4)
# met2df$STATIONID = substr(met2df$ID_HAUL, 5, lenstring)
# met3df = aggregate(met2df$C_A_I, list(met2df$YEAR, met2df$AGE), sum) # this is c-hat_a
# names(met3df) = c('YEAR', 'AGE', 'C_A')
# met4df = aggregate(met3df$C_A, list(met3df$YEAR), sum) # this is c-hat
# names(met4df) = c('YEAR', 'C')
...
# # met2xdf$NSAM = nSamLen$NSAM[match(met2xdf$YEAR, nSamLen$YEAR)]
# # met2xdf$INTVAR = met2xdf$C_A_I/met2xdf$NSAM # this is SE(c-hat_a)^2
# # met5df = aggregate(met2xdf$C_A_I, list(met2xdf$YEAR, met2xdf$AGE), var) # this is the numerator of SE(c-hat_a)^2
# # names(met5df) = c('YEAR', 'AGE', 'VAR_C_A')
write.csv(nSamLen, 'realData/nSamLen_pcod.csv')

# met5df = aggregate(met2df$C_A_I, list(met2df$YEAR, met2df$AGE), var) # this is the numerator of SE(c-hat_a)^2
# names(met5df) = c('YEAR', 'AGE', 'VAR_C_A')
# nSamLen = aggregate(data3$STATIONID, list(data3$YEAR), function(x) length(unique(x))) # should it be number of tows with len sample?
# names(nSamLen) = c('YEAR', 'NSAM')
# met5df$NSAM = nSamLen$NSAM[match(met5df$YEAR, nSamLen$YEAR)]
# met5df$SE_C_A = met5df$VAR_C_A/met5df$NSAM # this is SE(c-hat_a)^2
...
# Now, let's estimate SE(p_a)^2: (year by year)
# dffake = NULL
# for(i in seq_along(allYears)){
	
	# met1dftmp = met1df[met1df$YEAR == as.character(allYears)[i], ]
	# met3dftmp = met3df[met3df$YEAR == as.character(allYears)[i], ]
	# met4dftmp = met4df[met4df$YEAR == as.character(allYears)[i], ]
	# met5dftmp = met5df[met5df$YEAR == as.character(allYears)[i], ]
	# sepa = ((met3dftmp$C_A^2)/(met4dftmp$C^2))*(met5dftmp$SE_C_A/(met3dftmp$C_A^2) - 2*(met5dftmp$SE_C_A/(met3dftmp$C_A*met4dftmp$C)) + sum(met5dftmp$SE_C_A)/(met4dftmp$C^2))
	# median(met1dftmp$FREQUENCY*(1-met1dftmp$FREQUENCY)/sepa
...
write.csv(out1, 'realData/Method1_pcod.csv')
...
write.csv(out2, 'realData/Method2_pcod.csv')
...
write.csv(out4, 'realData/Method4_pcod.csv')
